        ...
        <div class="flex flex-row space-x-10">
            ...
            <div>•</div>
                <select v-model="colorMode.preference">
                    <option :v-for="mode in modeParams">{{ mode }}</option>
                </select>
            <div class="space-x-6">
                <Icon name="twemoji:flag-france" :class="{ active:!french }" @click="setLocale('fr'); french = true"/>
                <Icon name="twemoji:flag-united-kingdom" :class="{ active: french }"  @click="setLocale('en'); french = false"/>
            </div>
        </div>
    </div>
</template>